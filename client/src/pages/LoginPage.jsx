...
const LoginPage = () => {
  ...
  return (
    <div className='min-h-screen flex justify-center items-center bg-cover bg-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      {/* --------Left------- */}
      <img src={assets.big_icon} alt="Not Found" className='w-[25%]'/>
      {/* -------Right-------- */}
      <form onSubmit={onSubmitHandler} className='flex flex-col border-2 bg-white/8 text-white p-6 border-gray-500 gap-6 rounded-lg shadow-lg'>
        <h2 className='flex justify-between items-center font-medium text-2xl'>
          {currState}
          {isDataSubmitted && <img onClick={()=>setIsDataSubmitted(false)} src={assets.arrow_icon} alt="Not Found" className='w-5  cursor-pointer'/>}
        </h2>
        ...
        <button type='submit' className='py-3 bg-gradient-to-r from-purple-400 to-violet-600 rounded-md text-white cursor-pointer'>
          {currState === "Sign up" ? 'Create a new accoumt' : "Login"}
        </button>
